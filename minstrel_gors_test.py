...
def main():
    ...
    gors = kl_ucb_policy_minstrel.GORS(K, rate, his_group_succ_array, his_group_total_array, klucb_upper=kl_ucb_policy_minstrel.klucb_upper_bisection_with_l) #Optimal Graphical OptimalRate Sampling
    gors.reset()
    ...
    actions_gors = np.zeros((K, T), dtype=int)
    while (True):
        while (check_update(his_group_succ, his_group_total)):
            print("time is",t)
            arm_klucb = klucb.select_next_arm()
            arm_gors = gors.select_next_arm()
            # print("select arm is ", arm_klucb)
            print("select arm is ", arm_gors)
            echo_switch(arm_gors)
            # echo_switch(arm_klucb)
            # actions_klucb[arm_klucb, t] = 1
            actions_gors[arm_gors, t] = 1
            
            gors.update_state(np.array(his_group_succ), np.array(his_group_total))
            # klucb.update_state(np.array(his_group_succ), np.array(his_group_total))
            t += 1
